...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    conversationsApi.list(user.id).then(async (list) => {
      setConversations(list);
      setIsLoadingConversations(false);
      if (list.length > 0) {
        const firstId = list[0].id;
        setActiveId(firstId);
        const msgs = await conversationsApi.getHistory(firstId);
        if (msgs && msgs.length > 0) {
          setConversations((prev) =>
            prev.map((c) => (c.id === firstId ? { ...c, messages: msgs } : c))
          );
        }
      }
    });
  ...
  useEffect(() => {
    ...
    if (target && target.messages.length === 0) {
      conversationsApi.getHistory(activeId).then((msgs) => {
        if (msgs && msgs.length > 0) {
          setConversations((prev) =>
  ...
  const selectConversation = useCallback((id: string) => {
    setActiveId(id);
    conversationsApi.getHistory(id).then((msgs) => {
      if (msgs && msgs.length > 0) {
        setConversations((prev) =>
